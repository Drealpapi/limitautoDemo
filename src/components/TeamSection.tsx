...
export default function TeamSection() {
  return (
    <section
      ...
      style={{ background: '#0C1829', padding: '96px 0', position: 'relative', overflow: 'hidden' }}
    >
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 28px', position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <div style={{
            ...
            <Fa icon={Icons.users} style={{ fontSize: 12, color: '#3B82F6' }} />
            <span style={{ fontSize: 11, fontWeight: 700, color: '#60A5FA', letterSpacing: '0.16em', textTransform: 'uppercase' }}>
              Our Team
            </span>
          </div>
          ...
          <p style={{ fontSize: 16, color: 'rgba(148,175,210,0.75)', lineHeight: 1.7, maxWidth: 500, margin: '0 auto' }}>
        ...
        <div className="team-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
          {TEAM.map(member => (
            <div key={member.name} style={{
              ...
              onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(-5px)'; el.style.boxShadow = '0 16px 48px rgba(0,0,0,0.45)'; el.style.borderColor = 'rgba(37,99,235,0.3)'; }}
            ...
            >
              <div style={{ position: 'relative', aspectRatio: '3/4', overflow: 'hidden' }}>
                <img
                  ...
                  style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', display: 'block', filter: 'brightness(0.88) saturate(0.9)', transition: 'transform 0.45s ease' }}
                  onMouseEnter={e => ((e.currentTarget as HTMLElement).style.transform = 'scale(1.04)')}
                  onMouseLeave={e => ((e.currentTarget as HTMLElement).style.transform = 'scale(1)')}
                />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '45%', background: 'linear-gradient(to top, rgba(8,14,28,0.92) 0%, transparent 100%)', pointerEvents: 'none' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 16px 16px' }}>
                  <p style={{ fontSize: 15.5, fontWeight: 700, color: '#F1F5F9', lineHeight: 1.2, marginBottom: 4 }}>{member.name}</p>
                  <p style={{ fontSize: 12, fontWeight: 600, color: '#3B82F6', letterSpacing: '0.02em' }}>{member.role}</p>
                </div>
              </div>
              <div style={{ padding: '14px 16px 18px' }}>
                <p style={{ fontSize: 13, color: 'rgba(148,175,210,0.72)', lineHeight: 1.6 }}>{member.bio}</p>
              </div>
            </div>
          ))}
        </div>
